...
using namespace RooFit;
 ...
 ClassImp(RooLandauCB)

 RooLandauCB::RooLandauCB(){}

 RooLandauCB::RooLandauCB(const char *name, const char *title,
                    RooAbsReal& _x,
                    RooAbsReal& _mean,
                    RooAbsReal& _a,
                    RooAbsReal& _b,
 ...
 RooLandauCB::RooLandauCB(const RooLandauCB& other, const char* name) :
 ...
 double RooLandauCB::evaluate() const
 {

   const double t = (x-mean);
   double val = -99.;
   if(t > -alpha1 && t < alpha2) { // core Landau
     val = vdt::fast_exp(a*(b*t - vdt::fast_exp(b*t)));
   } else if(t <= -alpha1) { // left tail
     const double B1 = -alpha1 + n1*vdt::fast_inv(a*b*(1. - vdt::fast_exp(-b*alpha1)));
     const double A1 = vdt::fast_exp(a*(-b*alpha1-vdt::fast_exp(-b*alpha1)))*gbrmath::fast_pow(B1+alpha1,n1);
     val = A1*gbrmath::fast_pow(B1 - t, -n1);
   } else if(t >= alpha2) { // right tail
     const double B2 = -alpha2 - n2*vdt::fast_inv(a*b*(1. - vdt::fast_exp(b*alpha2)));
     const double A2 = vdt::fast_exp(a*(b*alpha2-vdt::fast_exp(b*alpha2)))*gbrmath::fast_pow(B2+alpha2,n2);
     val = A2*gbrmath::fast_pow(B2 + t, -n2);
   }

   if (!std::isnormal(val)) {
     printf("RooLandauCB: bad val: x = %5f, t = %5f, mean = %5f, a = %5f, b = %5f, alpha1 = %5f, n1 = %5f, alpha2 = %5f, n2 = %5f\n",
            double(x), t, double(mean), double(a), double(b), double(alpha1), double(n1), double(alpha2), double(n2));
     printf("--> val = %5f\n", val);
   }
 ...
 Int_t RooLandauCB::getAnalyticalIntegral(RooArgSet& allVars, RooArgSet& analVars, const char* range) const
 {
   return 0;
 ...
 Double_t RooLandauCB::analyticalIntegral(Int_t code, const char* rangeName) const
 {
   assert(code==1) ;
   return 0.;
